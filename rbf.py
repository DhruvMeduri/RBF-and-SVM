...
clusters = 19 # number of clusters
...
plt.ylabel("%error")
plt.title("Error Trajectory(Versicolor and Virginica - Clusters = 19)")
plt.show()

'''
#this code is the implementation of rbf to classify Setosa and Virginica
desired = np.zeros(100)
data = np.zeros((100,4))
err = []
for i in range(50):
...
plt.xlabel("Iteration")
plt.ylabel("%error")
plt.title("Error Trajectory(Setosa and Virginica - Clusters = 10)")
plt.show()
'''
'''
#this code is the implementation of rbf to classify Versicolor and Setosa
desired = np.zeros(100)
data = np.zeros((100,4))
err = []
for i in range(100):
...
for i in range(70,100):
    testing.append(lst[i])
#first we compute the k means for the data_lst
clusters = 16 # number of clusters
means = np.random.rand(clusters,4) # initilisation of the means
for i in range(clusters):
    means[i][0] = 6 + np.random.rand()
    means[i][1] = 2.5 +  np.random.rand()
...
plt.xlabel("Iteration")
plt.ylabel("%error")
plt.title("Error Trajectory(Versicolor and Setosa - Clusters = 16)")
plt.show()
'''
